# Packet: build the module 13–20 compositions (advanced arc)

You built modules 01–12 of this Remotion project. This adds the advanced arc, units 13–20, and completes a twenty-unit course.

## Read first

- `SERIES.md` — the series contract.
- `modules/u13-bootstrap/script.json` … `modules/u20-troubleshooting/script.json`
  — eight scripts, same beat shape as the modules you have already built. They
  are shorter: these units carry roughly half the content of units 1–12, so the
  modules run about three minutes each. That is correct, not missing material.
- `src/` — the project. A hundred and twenty-five visual components already exist. Reuse aggressively; several beats explicitly name an existing component.
- `modules/u12-scale-evidence/` — your last module. Match it.

## Scope

1. **Eight new compositions**, ids `Module13` … `Module20`, each reading only its
   own script.
2. **Register each in `src/Root.tsx`** exactly as `Module12` is registered,
...
- Do not draw RBAC as a switch defaulting off; it is already in the request path.
- Do not draw a topology-rejected Pod returning to Pending; it terminates Failed.
- Do not draw a previous container's log as always retrievable; it is garbage-collected.
...
1. `npx remotion compositions src/index.ts` showing twenty-one compositions, with
   modules 01–12 unchanged.
2. `node scripts/render-module-stills.mjs <each new module>`. **Never hard-code
   frame numbers.**
3. `node scripts/check-frames.mjs out/Module13-*.png` and the same for 14–20,
...
`MODULES 13-20 STATUS: READY` or `MODULES 13-20 STATUS: BLOCKED`
