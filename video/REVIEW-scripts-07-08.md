# Review request: technical accuracy of two video scripts
...
- `modules/u07-kubelet/script.json` — the kubelet, CRI/OCI/CNI/CSI, probes,
  termination, status reporting.
- `modules/u08-networking/script.json` — Service identity, EndpointSlices,
  NetworkPolicy, the data planes, the CNI exec contract, IPAM.
...
- Anything a CKA candidate genuinely needs that the script omits, given the
  ...
  module 09 is DNS, 10 is storage, 11 is etcd/HA, 12 is scale and API Priority
  and Fairness.
...
`u07 VERDICT: ACCURATE` or `u07 VERDICT: N CORRECTIONS NEEDED`
`u08 VERDICT: ACCURATE` or `u08 VERDICT: N CORRECTIONS NEEDED`
